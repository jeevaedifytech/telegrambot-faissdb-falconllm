...
#---- fias db with transformer models
# import logging
# from telegram import Update
...
# 1.Code for OpenAssistant Model:
...
# # tokenizer = AutoTokenizer.from_pretrained("OpenAssistant/oasst-sft-4-pythia-7b-epoch-3.5")
# # model = AutoModelForCausalLM.from_pretrained("OpenAssistant/oasst-sft-4-pythia-7b-epoch-3.5", device_map="auto")
...
# def find_answer(question, text_chunks, max_context_length=1024, max_new_tokens=150):
#     # Use chunks as context, truncated to fit within max_context_length
...
#         await update.message.reply_text("PDF uploaded and processed! You can now ask me questions about it.")
#     else:
#         await update.message.reply_text("The PDF contains no readable text or failed to process. Please check the file.")
...
#     text_chunks = split_text_into_chunks(context.user_data["pdf_text"])
#     try:
#         answer = find_answer(question, text_chunks)
#         await update.message.reply_text(answer)
#     except Exception as e:
...
##  Lama 2  7b, 13b, 70b   working   code
...
# Environment variables for tokens (better security)
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "7995379796:AAF_Imy8fPlh-gWUNXkckY5nexttTlSdpGA")
HF_TOKEN = os.getenv("HF_TOKEN", "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")
...
# Load LLaMA model
tokenizer = AutoTokenizer.from_pretrained("tiiuae/falcon-40b-instruct", use_auth_token=HF_TOKEN)
model = AutoModelForCausalLM.from_pretrained("tiiuae/falcon-40b-instruct", use_auth_token=HF_TOKEN, device_map="auto")
...
def find_answer(question, text_chunks, max_context_length=1024, max_new_tokens=300):
    # Use chunks as context, truncated to fit within max_context_length
    context = "\n".join(text_chunks)[:max_context_length]
    input_text = f"Context: {context}\nQuestion: {question}\nAnswer:"
